...
def test_supabase_connection():
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"[OK] Supabase 연결 성공. 테이블 접근 확인.")
            return True
        else:
...
def get_all_video_ids():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ video_id 조회 실패: {response.status_code} - {response.text}")
            return []
    except Exception as e:
...
def update_subtitle_in_db(video_id, subtitle_text):
    ...
    try:
        ...
        if response.status_code == 204:  # Supabase PATCH는 204를 반환
            ...
            print(f"❌ {video_id}: DB 업데이트 실패 - {response.status_code}: {response.text}")
            return False
...
def get_signals_for_timestamp_correction():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ 타임스탬프 교정 대상 시그널 {len(data)}개 조회")
            return data
        else:
            print(f"❌ 시그널 조회 실패: {response.status_code} - {response.text}")
            return []
    except Exception as e:
        print(f"❌ 시그널 조회 실패: {e}")
        return []
...
def update_signal_timestamp(signal_id, new_timestamp):
    ...
    try:
        ...
        if response.status_code == 204:
            ...
            print(f"❌ 시그널 {signal_id} 타임스탬프 업데이트 실패: {response.status_code}")
            return False
    except Exception as e:
        print(f"❌ 시그널 {signal_id} 타임스탬프 업데이트 실패: {e}")
        return False
        ...
        if current_timestamp is not None:
            ...
            if diff >= 30:
                ...
                if update_signal_timestamp(signal_id, new_timestamp_seconds):
                    print(f"✅ 시그널 {signal_id}: {current_timestamp}s → {new_timestamp_seconds}s (차이: {diff}s)")
                    correction_count += 1
    
    print(f"\n📊 타임스탬프 교정 완료: {correction_count}건")
