...
class TimestampTester:
    ...
    async def get_subtitles_from_yt_dlp(self, video_id: str) -> Optional[List[Dict]]:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                print(f"    yt-dlp 반환 코드: {result.returncode}")
                if result.stderr:
                ...
                if result.returncode == 0:
                    ...
                    # 파일 목록 확인
                    files = list(Path(temp_dir).glob(f'{video_id}*'))
                    print(f"    생성된 파일들: {[f.name for f in files]}")
    ...
    async def test_signals(self, limit: int = 1):
        """1개 시그널로 테스트 (수정됨)"""
        try:
            ...
            for i, signal in enumerate(signals):
                ...
                print(f"Current (초): {current_seconds}")
                
                # 자막 추출
                subtitles = await self.get_subtitles_from_yt_dlp(video_id)
                if not subtitles:
                    print(f"자막 추출 실패 - 다음으로 넘어감")
                    continue
                ...
                print(f"차이: {diff}초")
                print(f"교정 {'필요' if diff >= 30 else '불필요'}")
...
async def main():
    ...
    success = await tester.test_signals(1)
    if not success:
        print("테스트 실패 - yt-dlp 설정 문제일 가능성 있음")
