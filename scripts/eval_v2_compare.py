"""
V14.0 vs V14.1 프롬프트 Eval 비교 — 통합 eval set v2 (280건)

사용법:
  cd invest-sns
  PYTHONIOENCODING=utf-8 python scripts/eval_v2_compare.py
"""
...
PROMPT_V14_1 = """투자 인플루언서의 발언을 분석하여 시그널을 분류하세요.
시그널 타입: 매수/긍정/중립/부정/매도

■ 매수 = 강한 확신 + 투자 권유, 또는 실제 투자 행동 표현.
  "사라", "매수 추천", "추천한 거예요", "지금 들어가야", 본인 매매 공개.
  구체적 매매 전략 + "지금이 좋다": "DCA 전략으로 지금 가장 좋을 때다"
  ⚠️ 행동 동사 = 매수: 발언자가 실제 투자 행동을 했거나 하겠다는 표현 → 매수
    "직접 샀다/사고 있다", "포지션 잡았다/진입했다", "비중 늘렸다/추가 매수했다"
    "들어갔다/들어가고 있다", "담았다/모아가고 있다", "베팅했다"
    → 과거형/현재진행형 행동 동사가 있으면 단순 호감이 아니라 실제 투자 행위 = 매수
  ❌ "~해 보시면 좋을 것 같다", "담아보시면", "소개해 드리고 싶습니다" → 긍정

■ 긍정 = 호의적이지만 직접 매수 권유 없음, 행동 동사 없음.
  - 영상이 해당 종목을 직접 분석하며 성장성/매력을 호의적으로 서술 → 긍정
  - "좋은 종목", "전망 밝다", "관심 가져볼 만하다"
  - "~하면 좋겠다", "담아 보시면", "소개해 드리고 싶습니다" → 긍정 (매수 아님)
...
Q: 종목=엔비디아 / 영상="AI 반도체 포지션 공개"
발언: "저는 엔비디아 비중을 계속 늘려가고 있습니다"
A: {"signal_type": "매수"} ← "비중을 늘려가고 있다" = 실제 투자 행위 (행동 동사)

Q: 종목=테슬라 / 영상="전기차 시장 전망"
발언: "테슬라 포지션 잡았고 장기적으로 가져갈 겁니다"
A: {"signal_type": "매수"} ← "포지션 잡았다" = 실제 매수 행동

Q: 종목=삼성전자 / 영상="반도체 투자 전략"
발언: "삼성전자 정말 좋은 회사라서 저도 직접 샀습니다"
A: {"signal_type": "매수"} ← "직접 샀다" 행동 동사 = 매수. "좋은 회사"만 보고 긍정 처리하면 오답
...
def main():
    ...
    print("=" * 70)
    print(f"  V14.0 vs V14.1 — 통합 Eval v2 ({total}건)")
    print("=" * 70)
    ...
    for i, c in enumerate(cases):
        ...
        time.sleep(0.15)
        v141 = classify(PROMPT_V14_1, c["stock"], c["key_quote"], c["video_title"])

        correct = c["correct"]
        src = c.get("source", "?")[:3]

        def mark(r): return "O" if r == correct else "X"
        print(f"[{src}] 정답={correct:3s} V14.0={v140:3s}({mark(v140)}) V14.1={v141:3s}({mark(v141)})")

        v140_results.append(v140)
        v141_results.append(v141)
        time.sleep(0.2)

    v140_ok = sum(1 for i in range(total) if v140_results[i] == cases[i]["correct"])
    v141_ok = sum(1 for i in range(total) if v141_results[i] == cases[i]["correct"])

    print(f"\n{'=' * 70}")
    print(f"  RESULT ({total}건)")
    print(f"{'=' * 70}")
    print(f"  V14.0: {v140_ok}/{total} ({v140_ok/total*100:.1f}%)")
    print(f"  V14.1: {v141_ok}/{total} ({v141_ok/total*100:.1f}%)")
    delta = v141_ok - v140_ok
    print(f"  V14.0→V14.1: {'+' if delta>=0 else ''}{delta}건 ({'+' if delta>=0 else ''}{delta/total*100:.1f}%p)")
    ...
    for src in sorted(set(c.get("source", "?") for c in cases)):
        ...
        s141 = sum(1 for i in idxs if v141_results[i] == cases[i]["correct"])
        print(f"  [{src}] V14.0={s140}/{len(idxs)} ({s140/len(idxs)*100:.1f}%) V14.1={s141}/{len(idxs)} ({s141/len(idxs)*100:.1f}%)")

    # New corrections / regressions (V14.0 → V14.1)
    new_ok = [(i, cases[i]) for i in range(total) if v141_results[i] == cases[i]["correct"] and v140_results[i] != cases[i]["correct"]]
    new_ng = [(i, cases[i]) for i in range(total) if v141_results[i] != cases[i]["correct"] and v140_results[i] == cases[i]["correct"]]
    both_ok = sum(1 for i in range(total) if v141_results[i] == cases[i]["correct"] and v140_results[i] == cases[i]["correct"])
    both_ng = [(i, cases[i]) for i in range(total) if v141_results[i] != cases[i]["correct"] and v140_results[i] != cases[i]["correct"]]

    print(f"\n  Both correct: {both_ok}")
    print(f"  Both wrong:   {len(both_ng)}")

    if new_ok:
        print(f"\n  +++ V14.1 newly correct ({len(new_ok)}) +++")
        for idx, c in new_ok[:20]:
            cat = c.get('reason_cat') or c.get('source', '?')
            print(f"    {c['stock']}: V14.0={v140_results[idx]}->V14.1={v141_results[idx]} (ans={c['correct']}) [{cat}]")
        if len(new_ok) > 20:
            print(f"    ... and {len(new_ok)-20} more")
    if new_ng:
        print(f"\n  --- V14.1 regression ({len(new_ng)}) ---")
    ...
    v141_errors = [(i, cases[i]) for i in range(total) if v141_results[i] != cases[i]["correct"]]
    if v141_errors:
        pattern = Counter(f"{v141_results[i]}->{cases[i]['correct']}" for i, _ in v141_errors)
        print(f"\n  V14.1 error patterns ({len(v141_errors)} total):")
    ...
    out = {
        ...
        "v141": round(v141_ok/total*100, 1),
        ...
        "details": [
            {
                ...
                "v141_correct": v141_results[i] == cases[i]["correct"]
            }
            for i in range(total)
        ]
    }
    out_path = Path(__file__).parent.parent / "data" / "eval_v2_results.json"
