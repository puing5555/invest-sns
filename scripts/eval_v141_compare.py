"""
V14.0 vs V14.1 프롬프트 Eval 비교 (69건)

V14.1 핵심 변경:
  - "긍정→매수" 해결: 행동 동사 패턴 매수 조건 추가
  - "긍정→중립" 해결: 투자 의견 없는 언급 = 중립 룰 강화
  - few-shot 예시 7개 추가 (행동 동사 매수 3, 중립 3, 오분류 방지 1)

사용법:
  cd invest-sns
  PYTHONIOENCODING=utf-8 python scripts/eval_v141_compare.py
"""
...
def main():
    ...
    print("=" * 65)
    print(f"  V14.0 vs V14.1 Eval ({total}건)")
    print("=" * 65)
    ...
    for i, c in enumerate(cases):
        ...
        print(f"정답={correct:3s} V14.0={v140:3s}({mark(v140)}) V14.1={v141:3s}({mark(v141)})")
    ...
    if new_ok:
        ...
        for idx, c in new_ok:
            print(f"    {c['stock']}: V14.0={v140_results[idx]}->V14.1={v141_results[idx]} (ans={c['correct']}) [{c['reason_cat']}]")
    if new_ng:
    ...
    out_path = Path(__file__).parent.parent / "data" / "eval_v141_results.json"
