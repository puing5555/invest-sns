# -*- coding: utf-8 -*-
"""강환국 배치 파이프라인 (GPT-4o) — 영상 ID 리스트로 자막 추출 + AI 분석 + DB 저장.
Usage:
  python scripts/kang_batch_pipeline_gpt4o.py --part A
  python scripts/kang_batch_pipeline_gpt4o.py --part B
  python scripts/kang_batch_pipeline_gpt4o.py --part B --start 100
"""
...
def main():
    ...
    print(f'[파트{args.part}] GPT-4o | {len(video_ids)}개 영상, start={args.start}')
    ...
    for i in range(args.start, len(video_ids)):
        ...
        try:
            ...
            if api_calls > 0 and api_calls % 50 == 0:
                print(f'  --- 진행: api_calls={api_calls}, 시그널={total_signals} ---', flush=True)
                time.sleep(5)
            else:
                time.sleep(2)
    ...
    print(f'\n{"="*60}')
    print(f'파트{args.part} GPT-4o 완료: {processed}건 처리, {total_signals}건 시그널, {errors}건 에러')
